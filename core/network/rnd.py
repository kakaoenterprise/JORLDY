...
class RND_MLP_RNN(torch.nn.Module):
    def __init__(self, D_in, D_out, num_workers, gamma_i, 
                 ri_normalize=True, obs_normalize=True, batch_norm=True, D_hidden=256):
        super(RND_MLP_RNN, self).__init__()
        self.D_in = D_in
        self.D_out = D_out
        self.num_workers = num_workers
        self.rms_obs = RunningMeanStd(D_in)
        ...
        # Predictor Networks
        self.fc1_predict = torch.nn.Linear(self.D_in[1], self.D_hidden)
        self.lstm_predict = torch.nn.LSTM(input_size=self.D_hidden, hidden_size=self.D_hidden, batch_first=True)
        self.fc2_predict = torch.nn.Linear(self.D_hidden, self.D_hidden)
        self.fc3_predict = torch.nn.Linear(self.D_hidden, self.D_hidden)
        
        # Target Networks
        self.fc1_target = torch.nn.Linear(self.D_in[1], self.D_hidden)
        self.lstm_target = torch.nn.LSTM(input_size=self.D_hidden, hidden_size=self.D_hidden, batch_first=True)
        self.fc2_target = torch.nn.Linear(self.D_hidden, self.D_hidden)
        
        if batch_norm:
            self.bn1_predict = torch.nn.BatchNorm1d(self.D_hidden)
            self.bn1_target = torch.nn.BatchNorm1d(self.D_hidden)
    ...
    def forward(self, s_next, update_ri=False):
        ...
        s_next = s_next.reshape(-1, s_next.shape[2])
        ...
        p = p.view(-1, seq_len, self.D_hidden)
        t = t.view(-1, seq_len, self.D_hidden)
        
        p, hidden_p = self.lstm_predict(p, hidden)
        t, hidden_t = self.lstm_target(t, hidden)
...
class RND_CNN_RNN(torch.nn.Module):
    def __init__(self, D_in, D_out, num_workers, gamma_i, 
                 ri_normalize=True, obs_normalize=True, batch_norm=True, D_hidden=512):
        super(RND_CNN_RNN, self).__init__()
        self.D_in = D_in
        self.D_out = D_out
        self.num_workers = num_workers
        ...
        self.feature_size = 64*dim3[0]*dim3[1]
        ...
        self.lstm_predict = torch.nn.LSTM(input_size=self.feature_size, hidden_size=self.D_hidden, batch_first=True)
        
        self.fc1_predict = torch.nn.Linear(self.D_hidden, self.D_hidden)
        self.fc2_predict = torch.nn.Linear(self.D_hidden, self.D_hidden)
        self.fc3_predict = torch.nn.Linear(self.D_hidden, self.D_hidden)
        ...
        self.lstm_target = torch.nn.LSTM(input_size=self.feature_size, hidden_size=self.D_hidden, batch_first=True)

        self.fc1_target = torch.nn.Linear(self.D_hidden, self.D_hidden)
    ...
    def forward(self, s_next, update_ri=False):
        ...
        p = p.view(-1, seq_len, self.feature_size)
        p, hidden_p = self.lstm_predict(p, hidden)
        p = p[:,-1]
        ...
        t = t.view(-1, seq_len, self.feature_size)
        t, hidden_t = self.lstm_target(t, hidden)
        t = t[:,-1]
        t = self.fc1_target(t)
